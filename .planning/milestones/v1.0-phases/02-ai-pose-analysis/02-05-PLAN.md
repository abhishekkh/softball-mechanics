...
must_haves:
  truths:
    ...
    - "If analysis.status === 'error', a red error callout appears in the sidebar with the error message and a note that partial results are shown below"
  ...
  artifacts:
    - path: "src/components/review/MechanicsSidebar.tsx"
      provides: "Joint angles display + flag panel + Prev/Next navigation + toggle + Re-analyze button + error callout"
  ...
  key_links:
    ...
    - from: "src/app/(app)/review/[videoId]/page.tsx"
      ...
      via: "<MechanicsSidebar currentFrame angles flags analysisErrorMessage />"
...
export function usePoseAnalysis(
  ...
  analysisErrorMessage: string | null   // Non-null when status === 'error'
...
- **Error callout when analysisErrorMessage is non-null** (locked CONTEXT.md decision: "show partial results with a warning — do not hide data")
...
**Note on Joint Angles visibility:** The Joint Angles section now also renders when `analysisStatus === 'error'` — this ensures partial frames collected before the error are still displayed to the coach (consistent with the locked CONTEXT.md decision).
  </action>
  <verify>
  ...
  <done>
    ...
    - Props interface includes analysisErrorMessage: string | null
    - When analysisErrorMessage is non-null, a red callout renders: "Analysis error: {msg}. Showing partial results below. You can re-analyze using the button at the bottom."
    - Sidebar renders skeleton toggle, joint angles (with ideal ranges, shown for complete/low_confidence/error status), active flags, Prev/Next navigation, progress bar, framing warning, Re-analyze button
...
```
  ...
  <done>
    ...
    - ReviewPageClient destructures analysisErrorMessage from usePoseAnalysis() and passes it to MechanicsSidebar
    - MechanicsSidebar receives analysisErrorMessage prop and the error callout renders when it is non-null
...
<verification>
- src/components/review/MechanicsSidebar.tsx exports MechanicsSidebar with analysisErrorMessage: string | null in Props
- When analysisErrorMessage is non-null, a red callout renders in the sidebar BEFORE the framing warning
- MechanicsSidebar Joint Angles section renders for 'complete' | 'low_confidence' | 'error' status (not just the first two)
- src/app/(app)/review/[videoId]/page.tsx exists and imports ReviewPageClient
- src/components/review/ReviewPageClient.tsx destructures analysisErrorMessage from usePoseAnalysis() and passes it to MechanicsSidebar
...
5. When analysis.status === 'error', a red error callout appears in the sidebar: "Analysis error: {msg}. Showing partial results below. You can re-analyze using the button at the bottom." — partial frames are never suppressed
6. Dashboard SessionRow shows a "Review" link for ready videos navigating to /review/[videoId]
7. Upload page shows framing guidance tips above the file picker
